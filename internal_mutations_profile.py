...
def init_mismatch_dict(nucleotides='AUGC'):
    mismatch_dict = {}
    all_possible_mismatches = permutations(nucleotides, 2)
    all_possible_mismatches_list = []
    for i in all_possible_mismatches:
        string = '>'.join(i)
        if string.startswith('N'):
            continue  # this excludes the possibility where the reference is a 'N' nucleotide i.e ( N>[ACTG])
        all_possible_mismatches_list.append(string)
    for i in all_possible_mismatches_list:
        mismatch_dict[i] = 0
    mismatch_dict['Ns'] = 0
    return mismatch_dict


def update_internal_mm_dict(nte_string, nte_dict):
    ...
    for nte in nte_list:
        ...
        nte_nucleotides = re.sub('T', 'U', nte_change)
...
def make_mutation_composition_dict_order(nucleotides='AUGC'):
    ...
    mismatch_dict = init_mismatch_dict(nucleotides)

    count = 0
    order_dict = {}
    for i in mismatch_dict:
        order_dict[count] = i
        count += 1

    # print order_dict
    # order_dict= {0: 'G>U', 1: 'U>A', 2: 'U>C', 3: 'A>U', 4: 'G>A', 5: 'U>G', 6: 'G>C', 7: 'C>A', 8: 'A>G', 9: 'A>C', 10: 'C>U', 11: 'Ns', 12: 'C>G', 13: 'Ns'}
    return order_dict
...
####### INITIALIZING STUFF ###########
    ...
    NUMBER_NTES = 3
    NUCLEOTIDES = 'AUGC'
    output_order = make_mutation_composition_dict_order(NUCLEOTIDES)  # making a dictionary of different nucleotide NTE combinations of up to 3 nucleotides, key = NTE, value = read count, initializes value to 0
    num_combination = len(output_order)  # NTE dictionary space

    fout_obj_internal = open('{}.internal_mm_compsition'.format(fout), 'w')   # nte output files
...
#######    ACTUAL SCRIPT   ###########
    ...
    for i in fin_obj:
        ...
        internal_modification_dict = init_mismatch_dict(NUCLEOTIDES)  # initialized dict, key = NTE, value = count. inital val = 0

        i = i.strip().split()
        # print i

        internal_modification_string = i[16]
        internal_modification_dict = update_internal_mm_dict(internal_modification_string, internal_modification_dict)
...
def main():
    ...
    # x = init_mismatch_dict(nucleotides='AUGC')
    # make_mutation_composition_dict_order(nucleotides='AUGC')
    # print x
