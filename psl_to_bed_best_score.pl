...
while (<IN>){
   ...
   # my $score = ($sizeMul * ($matches + $repMatches)) - ($sizeMul * $misMatches) - $qNumInsert - $tNumInsert; # Original
   my $score = ($sizeMul * ($matches)) - ($sizeMul * $misMatches) - $qNumInsert - $tNumInsert; # - KIKU MODIFIED, removed rep_matches.
...
#header
print OUT "#chrom\ttStart\ttEnd\tname\tquerysize\tmatches\tscore\tstrand\tqStart\tqEnd\tthickStart\tthickEnd\titemRgb\tblockCount\tblockSizes\tblockStarts\n";
...
foreach my $query (keys %psl_result){
   ...
   foreach my $score (keys %{$psl_result{$query}}){
      ...
      for (my $i = 0; $i < scalar (@{$psl_result{$query}{$score}}); ++$i){
         ...
         $relative_t_starts =~ s/,$//;
         #BED  chrom,chromStart,chromEnd,name, querysize, matches, score ,strand ,thickStart,thickEnd,itemRgb,blockCount ,blockSizes ,blockStarts
         print OUT join("\t", $tName, $tStart, $tEnd, $qName, $qSize, $matches, $score, $strand, $qStart,$qEnd,$tStart, $tEnd, $red, $blockCount, $blockSizes, $relative_t_starts),"\n";
      }
   }
}
